/**
 * Bayesian Optimisation — GP-UCB for V10 (Ultra Low Latency)
 *
 * V10 fast-exits for wallets with ≤100 txns — sparse and medium are already
 * at the 1-RT floor. The only remaining knobs that matter are:
 *
 *   windowSize     [20, 100]  — affects dense wallet Phase 1 call count
 *   maxConcurrency [4,  32]   — affects dense wallet parallel throughput
 *   skipZeroDelta  {0,  1}    — affects all wallets
 *
 * anchor is fixed at 100 (maximises fast-exit hit rate).
 *
 * Usage:
 *   node research_bayes_v10.mjs <api-key> [budget=40]
 */

import { writeFileSync, mkdirSync } from "fs";
import { evaluate } from "../eval/eval_v10.mjs";

const apiKey = process.argv[2] ?? process.env.HELIUS_API_KEY;
const BUDGET = parseInt(process.argv[3] ?? "40", 10);

if (!apiKey) { console.error("Usage: node research_bayes_v10.mjs <api-key> [budget=40]"); process.exit(1); }
...
mkdirSync("results_bayes_v10", { recursive: true });
console.log(`\nBayesian Optimisation (GP-UCB) — V10 Ultra\n   Budget: ${BUDGET}  |  3D  |  ${apiKey.slice(0, 8)}...`);
console.log(`   anchor=100 fixed (fast-exit). Only dense wallet (286 txns) drives window/concurrency params.`);
...
for (let trial = 0; trial < BUDGET; trial++) {
  ...
  console.log(`\n[BO-V10 trial ${trial + 1}/${BUDGET}  k=${kappa.toFixed(2)}]`);
  ...
  writeFileSync("results_bayes_v10/log.json", JSON.stringify(logs, null, 2));
}

const best = logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score)[0];
console.log(`\n${"=".repeat(60)}\nBAYESIAN OPTIMISATION V10 COMPLETE\n${"=".repeat(60)}`);
...
if (best) writeFileSync("results_bayes_v10/best_strategy.json", JSON.stringify({
  _meta: { method: "bayesian-gp-ucb-v10-ultra", trial: best.trial, score: best.score,
           note: "anchor fixed at 100 for fast-exit" },
  ...best.strategy,
}, null, 2));

console.log("\nTop 5:");
for (const r of logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score).slice(0, 5))
  console.log(`  #${r.trial}  score=${r.score.toFixed(4)}  window=${r.strategy.windowSize}  c=${r.strategy.maxConcurrency}  skip=${r.strategy.skipZeroDelta}  ${r.aggregate?.avgLatencyMs?.toFixed(0)}ms`);
