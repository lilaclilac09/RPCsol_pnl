/**
 * Bayesian Optimisation — GP-UCB for V8 (Streaming Pipeline)
 *
 * Parameter space (4D):
 *   x[0] anchorSize     [10, 100]
 *   x[1] windowSize     [20, 100]
 *   x[2] maxConcurrency [4,  32]   ← wider upper bound than V3 (streaming may benefit more)
 *   x[3] skipZeroDelta  {0, 1}
 *
 * V8 hypothesis: optimal concurrency may be HIGHER than V3's c=23 because
 * Phase 1 starts earlier (Phase 0 is effectively shorter), so the window-fetch
 * bottleneck is more pronounced — more parallelism can help.
 *
 * Usage:
 *   node research_bayes_v8.mjs <api-key> [budget=40]
 */

import { writeFileSync, mkdirSync } from "fs";
import { evaluate } from "../eval/eval_v8.mjs";

const apiKey = process.argv[2] ?? process.env.HELIUS_API_KEY;
const BUDGET = parseInt(process.argv[3] ?? "40", 10);

if (!apiKey) { console.error("Usage: node research_bayes_v8.mjs <api-key> [budget=40]"); process.exit(1); }

const PARAMS = [
  { name: "anchorSize",     lo: 10, hi: 100, integer: true  },
  { name: "windowSize",     lo: 20, hi: 100, integer: true  },
  { name: "maxConcurrency", lo: 4,  hi: 32,  integer: true  },  // wider than V3
...
mkdirSync("results_bayes_v8", { recursive: true });
console.log(`\n🔬 Bayesian Optimisation (GP-UCB) — V8 Streaming\n   Budget: ${BUDGET}  |  4D  |  ${apiKey.slice(0, 8)}...`);
console.log(`   Key difference: maxConcurrency upper bound = 32 (wider than V3's 24)`);
...
for (let trial = 0; trial < BUDGET; trial++) {
  ...
  console.log(`\n[BO-V8 trial ${trial + 1}/${BUDGET}  κ=${kappa.toFixed(2)}]`);
  console.log(`  anchor=${strategy.anchorSize}  window=${strategy.windowSize}  c=${strategy.maxConcurrency}  skip=${strategy.skipZeroDelta}`);
  ...
  writeFileSync("results_bayes_v8/log.json", JSON.stringify(logs, null, 2));
}

const best = logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score)[0];
console.log(`\n${"═".repeat(60)}\nBAYESIAN OPTIMISATION V8 COMPLETE\n${"═".repeat(60)}`);
...
if (best) writeFileSync("results_bayes_v8/best_strategy.json", JSON.stringify({
  _meta: { method: "bayesian-gp-ucb-v8-streaming", trial: best.trial, score: best.score },
  ...best.strategy,
}, null, 2));

console.log("\nTop 5:");
for (const r of logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score).slice(0, 5))
  console.log(`  #${r.trial}  score=${r.score.toFixed(4)}  anchor=${r.strategy.anchorSize}  window=${r.strategy.windowSize}  c=${r.strategy.maxConcurrency}  ${r.aggregate?.avgLatencyMs?.toFixed(0)}ms`);
