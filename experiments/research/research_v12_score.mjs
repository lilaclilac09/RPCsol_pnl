/**
 * V12 Score-Max — Bayesian GP-UCB
 * Objective: maximise (completeness * 1000 / avgLatencyMs) * gap_penalty
 * Usage: node research_v12_score.mjs <api-key> [budget=50]
 */
import { writeFileSync, mkdirSync } from "fs";
import { evaluate } from "../eval/eval_v12_score.mjs"; // uses V11 solver, standard score

const apiKey = process.argv[2] ?? process.env.HELIUS_API_KEY;
const BUDGET = parseInt(process.argv[3] ?? "50", 10);
if (!apiKey) { console.error("Usage: node research_v12_score.mjs <api-key> [budget=50]"); process.exit(1); }
...
mkdirSync("results_v12_score", { recursive: true });
console.log(`\nV12 Score-Max (GP-UCB) — Budget ${BUDGET}  4D  ${apiKey.slice(0, 8)}...`);
console.log(`  Dual-anchor. Objective: completeness*1000/avgLatencyMs * gap_penalty`);
...
for (let trial = 0; trial < BUDGET; trial++) {
  ...
  console.log(`\n[Score trial ${trial + 1}/${BUDGET}  k=${kappa.toFixed(2)}]`);
  ...
  writeFileSync("results_v12_score/log.json", JSON.stringify(logs, null, 2));
}

const best = logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score)[0];
console.log(`\n${"=".repeat(60)}\nV12 SCORE-MAX COMPLETE\n${"=".repeat(60)}`);
...
if (best) writeFileSync("results_v12_score/best_strategy.json", JSON.stringify({
  _meta: { method: "bayesian-gp-ucb-v12-score-max", trial: best.trial, score: best.score },
  ...best.strategy,
}, null, 2));

console.log("\nTop 5:");
for (const r of logs.filter(l => l.score > 0).sort((a, b) => b.score - a.score).slice(0, 5))
  console.log(`  #${r.trial}  score=${r.score.toFixed(4)}  w=${r.strategy.windowSize}  c=${r.strategy.maxConcurrency}  skip=${r.strategy.skipZeroDelta}  ${r.aggregate?.avgLatencyMs?.toFixed(0)}ms`);
