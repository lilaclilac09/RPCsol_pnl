/**
 * eval_v15 — single-run evaluation for V15 (free-tier solver)
 *
 * V15 uses getSignaturesForAddress + parallel getTransaction calls.
 * Score formula with gate penalty (consistent with V13/V14):
 *   base  = (completeness × 1000 / avgLatencyMs) × max(0.1, 1 − 0.05 × openGapPenalty)
 *   gate  = if ANY wallet ≥ 3000ms → ×0.05  |  ≥ 2000ms → ×0.60  |  else 1.0
 *   score = base × gate
 */
import { solBalanceOverTime } from "../sol_balance/sol_balance_v15.mjs";
  ...
  for (const wallet of TEST_WALLETS) {
    try {
      ...
      results.push({
        ...
        rpcCalls: 999, sampleCount: 0, openGaps: 99, complete: false, error: err.message,
      });
    }
  }
