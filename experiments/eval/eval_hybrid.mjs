/**
 * eval_hybrid — evaluator for the hybrid density-aware router.
 *
 * Uses solBalanceFreeTierOnly (always free path, for BO tuning).
 * Reports wallet density classification and API tier per wallet.
 *
 * Score formula: identical to eval_v14/v15
 *   base  = (completeness × 1000 / avgLatencyMs) × max(0.1, 1 - 0.05 × openGapPenalty)
 *   gate  = maxWalletMs ≥ 3000ms → 0.05 | ≥ 2000ms → 0.60 | else → 1.0
 *   score = base × gate
 */

import { solBalanceFreeTierOnly } from "../sol_balance/sol_balance_hybrid.mjs";
  ...
  for (const wallet of TEST_WALLETS) {
    try {
      ...
      results.push({
        ...
        detectedDensity: result.stats.walletDensity,
      });
    } catch (err) {
      results.push({
        wallet: wallet.address, type: wallet.type, wallMs: 120000,
        rpcCalls: 60, sampleCount: 0, openGaps: 99, complete: false,
        detectedDensity: "error", error: err.message,
      });
    }
  }
