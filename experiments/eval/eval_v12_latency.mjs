/**
 * eval_v12_latency — objective: minimise worst-case per-wallet latency
 *
 * Score = completeness * 3000 / maxWalletMs * max(0.1, 1 - 0.05 * openGapPenalty)
 *
 * Using maxWalletMs (bottleneck) instead of avgLatencyMs forces BO to cut
 * down the SLOWEST wallet (dense at ~3000ms) rather than averaging wins
 * on fast wallets. Every wallet must complete quickly — not just the mean.
 */
// Uses V11 solver (dual-anchor V12 tested and regressed — extra call hurts free tier)
import { solBalanceOverTime } from "../sol_balance/sol_balance_v11.mjs";
...
export async function evaluate(strategy, apiKey) {
  ...
  // Bottleneck score: rewards minimising the slowest wallet
  const score = completeness > 0
    ? (completeness * 3000 / maxWalletMs) * Math.max(0.1, 1 - 0.05 * openGapPenalty)
    : 0;
  return { strategy, results, aggregate: { avgLatencyMs, maxWalletMs, totalRpc, openGapPenalty, completeness, score } };
}
