/**
 * SOL Balance V11 — Hybrid Ultra (V4 + V10)
 *
 * Combines every optimisation discovered across all versions:
 *
 * From V10 (Ultra Low Latency):
 *   - anchor=100 always (maximises fast-exit coverage)
 *   - Streaming: await sigs first, start Phase 1 windows immediately
 *   - Fast-exit: if anchor's newest slot >= sigs' newest slot → 1 serial RT
 *   - Map-based dedup O(1), in-place reverse for sigs array
 *
 * From V4 (Hybrid Density):
 *   - For wallets >sigPageSize txns: single blockTime density probe instead of
 *     sequential sig pagination — always 2 serial RTs regardless of wallet size
 *   - Both exact (gap < 1000 sigs) and density (gap >= 1000 sigs) paths
 *
 * Serial RT map:
 *   Wallet with ≤100 txns   → 1 RT  (anchor fast-exit)
 *   Wallet with ≤1000 txns  → 2 RTs (streaming sigs + parallel exact windows)
 *   Wallet with >1000 txns  → 2 RTs (Phase 0 + density probe Phase 1)
 *
 * V4's old weakness: Promise.all waited for BOTH calls; no streaming.
 * V11 fix: await sigs first; if not truncated, fire windows before anchor returns.
 *
 * V10's weakness: large wallets fall to sequential gap pagination (many serial RTs).
 * V11 fix: kept V4's density probe for the >sigPageSize case.
 */
...
if (_isCLI) {
  ...
  if (!address || !apiKey) { console.error("Usage: node sol_balance_v11.mjs <address> <api-key>"); process.exit(1); }
